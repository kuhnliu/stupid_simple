...
// are processed.
class QUIC_EXPORT_PRIVATE MyQuicFramerVisitorInterface {
 public:
  virtual ~MyQuicFramerVisitorInterface() {}

  // Called if an error is detected in the QUIC protocol.
  virtual void OnError(MyQuicFramer* framer) = 0;
...
class QUIC_EXPORT_PRIVATE MyQuicFramer {
 public:
  ...
  // will be used.
  void set_visitor(MyQuicFramerVisitorInterface* visitor) { visitor_ = visitor; }
...
public:
  ...
  MyQuicFramerVisitorInterface* visitor_;
  ...
  DISALLOW_COPY_AND_ASSIGN(MyQuicFramer);
};
